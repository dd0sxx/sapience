...
contract ParlayPoolIntegrationTest is Test {
    ...
    function testComplexParlayWithMultipleMarkets() public {
        ...
        IParlayStructs.PredictedOutcome[] memory outcomes = new IParlayStructs.PredictedOutcome[](3);
        outcomes[0] = IParlayStructs.PredictedOutcome({
    ...
    function testMultipleExpiredOrders() public {
        ...
        (IParlayStructs.ParlayData memory request1, ) = pool.getParlayOrder(requestId1);
        (IParlayStructs.ParlayData memory request2, ) = pool.getParlayOrder(requestId2);
        (IParlayStructs.ParlayData memory request3, ) = pool.getParlayOrder(requestId3);
    ...
    function settleMarketsForParlay(uint256 parlayId) internal {
        ...
        for (uint256 i = 0; i < outcomes.length; i++) {
            ...
            mockSapience.setMarketData(marketId, true, expectedOutcome, 0, 1000);
        }
    }
    ...
    function settleMarketsForParlayWithOutcome(uint256 parlayId, bool makerWins) internal {
        ...
        for (uint256 i = 0; i < outcomes.length; i++) {
            ...
            bool actualOutcome = makerWins ? prediction : (i == 0 ? !prediction : prediction);
