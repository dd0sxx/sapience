...
export const useUniswapPool = (chainId: number, poolAddress: `0x${string}`) => {
  ...
  const { data: token0Balance, refetch: refetchTokenBalance, isRefetching: isRefetchingUniswap } = useReadContract({
  ...
  useEffect(() => {
    if (data && token0Balance !== undefined) {
      ...
      if (token0Address && token1Address) {
        ...
        const token0 = new Token(chainId, token0Address as string, 18, 'Ggas', 'Ggas');
        const token1 = new Token(chainId, token1Address as string, 18, 'wstETH', 'wstETH');
        const poolInstance = new Pool(token0, token1, fee as FeeAmount, sqrtPriceX96.toString(), (uniswapLiquidity as any).toString(), tick);
