...
interface GetResourceCandlesQuery { resourceCandles: CandleAndTimestampType }
interface GetIndexCandlesQuery { indexCandles: CandleAndTimestampType }
...
export const useLatestResourcePrice = (slug: string) => {
  return useQuery<{ timestamp: string; value: string }>({
    ...
    queryFn: async () => {
      const from = Math.floor(Date.now() / 1000) - 300;
...
export const useLatestIndexPrice = (market: { address: string; chainId: number; marketId: number; }) => {
  return useQuery<{ timestamp: string; value: string } | null>({
    ...
    queryFn: async () => {
      ...
      const to = Math.floor(Date.now() / 1000);
      const interval = 60;
      const data = await graphqlRequest<GetIndexCandlesQuery>(LATEST_INDEX_PRICE_QUERY, {
