...
var createGraphQLClient = () => new GraphQLClient(getGraphQLEndpoint());
...
import { useReadContract as useReadContract2, useReadContracts } from "wagmi";
...
var useUniswapPool = (chainId, poolAddress) => {
  ...
  const { data, isError, isLoading } = useReadContracts({
    contracts: [
      { address: poolAddress, abi: IUniswapV3PoolABI.abi, functionName: "token0", chainId },
      { address: poolAddress, abi: IUniswapV3PoolABI.abi, functionName: "token1", chainId },
      { address: poolAddress, abi: IUniswapV3PoolABI.abi, functionName: "fee", chainId },
      { address: poolAddress, abi: IUniswapV3PoolABI.abi, functionName: "liquidity", chainId },
      { address: poolAddress, abi: IUniswapV3PoolABI.abi, functionName: "slot0", chainId }
    ]
  });
  const { data: token0Balance, refetch: refetchTokenBalance, isRefetching: isRefetchingUniswap } = useReadContract2({
  ...
  useEffect2(() => {
    if (data && token0Balance !== void 0) {
      ...
      if (token0Address && token1Address) {
        ...
        const token0 = new Token(chainId, token0Address, 18, "Ggas", "Ggas");
        const token1 = new Token(chainId, token1Address, 18, "wstETH", "wstETH");
        const poolInstance = new Pool(token0, token1, fee, sqrtPriceX96.toString(), uniswapLiquidity.toString(), tick);
