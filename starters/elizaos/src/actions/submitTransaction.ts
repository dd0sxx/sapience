...
export const submitTransactionAction: Action = {
  ...
  handler: async (
    ...
    try {
      ...
      const tx = JSON.parse(jsonMatch[0]) as { to: `0x${string}`; data?: `0x${string}`; value?: string };

      const privateKey = (process.env.EVM_PRIVATE_KEY || process.env.PRIVATE_KEY || process.env.WALLET_PRIVATE_KEY) as `0x${string}` | undefined;
      ...
      elizaLogger.info('[SUBMIT_TRANSACTION] Sending transaction', { to: tx.to } as any);
      ...
      await callback?.({ text: `Submission failed: ${err?.message}`, content: {} });
    }
  },
};
